 ...
 *
 * The numbers below are quoted from `docs/design/DESIGN-DECISIONS.md`; Sprint 02 moves them into
 * `src/core/settings.js`, which becomes the only place they may be edited.
 */

/** Arena size in cells; one cell is one world unit (DESIGN-DECISIONS §2.1). */
const ARENA = { width: 24, height: 24 };

/** Locked gameplay camera: 32° vertical FOV, 78° below the horizon, yaw 0 (DESIGN-DECISIONS §1 row 24). */
const CAMERA = { fov: 32, pitchDegrees: 78, margin: 1.5 };
...
floor.position.set(ARENA.width / 2, 0, ARENA.height / 2);
...
cube.position.set(ARENA.width / 2, CUBE_SIZE / 2, ARENA.height / 2);
...
keyLight.position.set(ARENA.width * 0.25, 30, ARENA.height * 0.9);
keyLight.target.position.set(ARENA.width / 2, 0, ARENA.height / 2);
keyLight.castShadow = true;
keyLight.shadow.mapSize.set(SHADOW_MAP_SIZE, SHADOW_MAP_SIZE);
keyLight.shadow.camera.left = -ARENA.width;
keyLight.shadow.camera.right = ARENA.width;
keyLight.shadow.camera.top = ARENA.height;
keyLight.shadow.camera.bottom = -ARENA.height;
...
function frameArena(target) {
  const halfExtent = Math.max(ARENA.width, ARENA.height) / 2 + CAMERA.margin;
