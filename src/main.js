...
import { createErrorScreen, watchContextLossForErrorScreen } from './ui/screens/error.js';
 ...
 *
 * **KI-06-02: the last-resort screen is built before anything that can fail, and everything that can fail is
 * inside the `try` below.** `createGameplayRenderer` throws when no WebGL context can be created at all
 * (three throws from its own constructor) — the same `try`/`catch` this ticket needs for "a thrown error
 * anywhere in startup" also catches that, which is the whole point of the ticket pairing the two ACs: there
 * is exactly one code path, not two. Nothing in the `catch` below assumes `ui`, `session` or `renderer` exist,
 * because any one of them may be the thing that threw.
 */

const canvas = /** @type {HTMLCanvasElement} */ (document.getElementById('game'));
const uiRoot = /** @type {HTMLElement} */ (document.getElementById('ui'));

// Built first, unconditionally: this is what a WebGL-unavailable or startup-throw load has to show *instead
// of* a blank page, so it must exist before `createGameplayRenderer` below gets a chance to be the thing that
// throws. `error.js`'s own module doc comment covers why this needs nothing but the DOM to exist.
const errorScreen = createErrorScreen(uiRoot);
...
try {
  ...
  // KI-06-02, the third path: a context that is lost and never comes back. `session.js` already pauses the
  // match on the same two events (KI-06-01); this only decides whether the last-resort screen goes up on top
  // of that already-frozen picture once `CONTEXT_LOSS_GRACE_MS` (`error.js`) has passed with no restore.
  watchContextLossForErrorScreen(renderer, errorScreen);
  ...
  // KI-06-02 AC2: a thrown error anywhere above lands here rather than in the console alone.
  // The real detail still goes to the console — whoever files the bug needs it — but the
  // player only ever sees the plain screen (`error.js`'s own module doc comment on why no
  // literal, technical detail belongs on it).
  console.error(err);
  errorScreen.show();
}
