...
| 27 | Playing alone | **A CPU opponent in the two-player mode** (Improvement 12): 2 PLAYERS gains a per-player HUMAN / CPU choice on match setup, defaulting to HUMAN for both. The CPU plays through the same input queue as a keyboard, at three named levels (EASY = the greedy bot, NORMAL = the survivor bot, HARD = the survivor bot that also eats when it is safe — a level that steered into head-ons measured 3.5 points *weaker* than NORMAL, #228; if HARD still does not beat NORMAL by a clear margin, two levels ship and HARD stays off the menu), and never sees anything a human cannot. Keys are awarded only when at least one human is playing. Solo survival (row 16) is unchanged and still post-1.0. | Design-lead ruling 2026-09-07; the owner may veto. A child alone should be able to play the game that exists. |
