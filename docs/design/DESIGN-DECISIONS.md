...
| 24 | Camera | **Perspective camera, vertical FOV 32°, pitch 78° below horizontal** (image 03 panel C), yaw 0, fixed position framing the whole arena plus one wall thickness of margin. Small shake on crash, ≤ 2 % zoom pulse on laser warning, no rotation. **Confirmed picture (Sprint 03):** at 16:9 the arena fills ≈ 84 % of the frame height and ≈ 51 % of its width (measured 50.8 % by Improvement 16; the figure read "≈ 48 %" until 2026-09-08 and the picture has not changed since Sprint 03); the flanks carry Sprint 09's decoration, as in panel C, not the wider board of image 02. | Locked by the camera comparison sheet. |
...
| 30 | Round pacing levers | **Measured, decision deferred to Gate 1 (Improvement 04, #229).** Over 33 cells and 12 299 seeded rounds: `roundDuration` moves the fraction of rounds reaching the lasers most (90→75 s: +16 to +23 points) but by arithmetic — it deletes play that would not have reached them; `laserStartTime` 40 s converts existing play into climax (+9 to +16 points) at unchanged match length; `laserStartTime` 20 s is a cliff (lasers stall at inset 7, 80 % of careful rounds time out) and is excluded as a value. **No settings change.** Both candidates ship as tuning-overlay presets beside the Speed Boost pair — `round 75 s` and `laser at 0:40` — and Gate 1 session 2 decides against `§5 A2` and `§6 R1`. The design lead's lean if humans cannot tell them apart: `laserStartTime: 40`, which keeps the GDD's round and makes the closing phase 44 % of it. **Addendum 2026-09-08 (KI-04-03, 39 cells):** the earlier-start arm measured to 45 and 50 s keeps climbing (greedy vs greedy 18.5 → 27.1 → 42.5 % of rounds reaching the warning) with the median round unmoved — because the median greedy round is 31 s, so bots rarely live long enough to pay for an early warning. Humans do. The chip stays at 0:40; 45 s is the measured next candidate if Gate 1 asks for more. | Bots die more cheaply than people, so the human baseline is above the bots' 9.5 % and the change needed is probably smaller than any cell implies; a permanent value must be chosen against the right population. |
| 31 | Framing a window that is not 16:9 | **The confirmed picture of row 24 is a *minimum*, not the only shape.** The camera keeps its locked parameters at every viewport — vertical FOV 32°, pitch 78° below horizontal, yaw 0, no rotation — and distance remains the only free variable. It is solved so that the whole arena **plus one wall thickness of margin** (`camera.margin`, 1.5 cells) is inside the frame on the axis that binds, which is always the shorter one: above an aspect ratio of ≈ 1.023 the fixed vertical FOV binds and the distance does not move with the aspect at all; below it the width takes over and the camera stands further back. Nothing is ever cropped at any aspect ratio, and no viewport is given *less* arena than 16:9 gets. **At 16:9 this resolves to exactly today's numbers** and is unchanged in every respect: distance 48.8581, camera at (12, 47.7904, 22.1582), the arena filling 84.0 % of the frame height and 50.8 % of its width, with the framed box's near edge sitting on the bottom of the frame to within one floating-point ULP. A wider viewport is that same picture with more empty flank; a squarer one is that same picture with more empty flank above and below. Two consequences of the fit being *exact* are recorded rather than fixed: row 24 stated the width as "≈ 48 %" where it measures 50.8 %, and the design lead corrected the figure on 2026-09-08 (the picture is unchanged since Sprint 03 — the number was what was imprecise), and at the peak of the ≤ 2 % laser-warning zoom pulse the margin is briefly clipped (worst 1.021) while the arena and the whole wall ring stay comfortably inside (worst 0.902). Widening the framing to absorb the pulse would move the camera and change the confirmed picture, which is forbidden. | Improvement 16 (#245, KI-16-02). Every proof in the project was at 1280×720; `solveCameraDistance` already took whichever constraint needed more room, so the rule was right — but it was written down nowhere and asserted nowhere except at 16:9 and 4:3, which is how a correct rule comes to be believed rather than checked. The seven-viewport matrix is `docs/qa/playtests/viewports.md` (KI-16-01). |
