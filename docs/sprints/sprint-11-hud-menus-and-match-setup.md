# Sprint 11 — HUD, Menus & Match Setup

**Lead:** Sonnet · **Agents:** Sonnet ×2, Sonnet-QA ×1, Fable · **Prerequisite:** `gate1-passed` (may run in parallel with S08–S10; needs `sprint-09-done` only for the final menu screenshots)
...
## Out of scope
Audio (S12), shop overlay (S14), tutorial bubble (S15), the 3D title mesh (the title is CSS/SVG; a 3D mesh is a
post-1.0 nicety).

## Tickets

### KS-11-01 · Toy UI design system (CSS)
...
top band is above the far wall, bottom pips sit on the near wall (verify with the camera framing from S03).
Acceptance criteria:
...
### KS-11-03 · Main menu
Owner: Sonnet · Size: M · Depends on: KS-11-01, KS-09-06
...
(zeros until S13). Focus = brighter border + slight scale. Keyboard hint line at the bottom ("↑↓ move · Enter
select"). Menu background scene from S09 behind.
Acceptance criteria:
...
### KS-11-05 · Countdown, scoreboard, match over, pause, settings shells
Owner: Sonnet · Size: M · Depends on: KS-11-01
...
variant. Match over: "RED WINS THE MATCH!" in the winner colour, key icon + "+N KEYS" (animation in S13),
REMATCH / MAIN MENU buttons. Pause: dim overlay, RESUME / RESTART MATCH / QUIT TO MENU; "READY?" flash on
resume. Settings: MUSIC ON/OFF, SOUND ON/OFF pills (wired in S12), BACK.
Acceptance criteria:
...
### KS-11-06 · UI accessibility pass
Owner: Sonnet-QA · Size: S · Depends on: KS-11-03, KS-11-05
...
## Exit criteria
- [ ] Fidelity verdict positive; a11y green; tag `sprint-11-done`.
