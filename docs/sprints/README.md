...
| [I04](improvement-04-round-pacing-and-the-climax.md) | Round pacing & the climax | Fable | **F3**: only 4 of 27 rounds lasted long enough to see a laser. **Complete 2026-09-08** (#229, `round-pacing.md`, 39 cells, 14 509 rounds): decision deferred to Gate 1 with three tuning-overlay presets (`round 75 s`, `laser at 0:40`, shipping), row 30 and its addendum; the earlier-start arm never turns over on bots, which is the population caveat in action; `settings.js` untouched, no golden moved | — |
| [I05](improvement-05-replay-capture-and-playback.md) | Replays you can watch | Sonnet | KS-07-01 records replays with nowhere to play them back. **Complete 2026-09-08** (#211): versioned format (absent `version` is 1, every fixture and I11 export unchanged), a player that replays from tick 0 and seeks exactly, a REPLAY menu row and screen with approved copy, WATCH LAST ROUND on match over; five real bot rounds recorded and replayed tick for tick. KI-05-05 makes the three timing-sensitive e2e tests deterministic (#238, #235) | — |
| [I06](improvement-06-resilience-and-recovery.md) | Resilience & recovery | Opus | Nothing handles WebGL context loss; a sleeping laptop kills the match. **Started 2026-09-08** (#276) | — |
...
| [I08](improvement-08-performance-budgets-before-the-art.md) | Performance budgets | Opus | `ARCHITECTURE §12` states budgets nothing enforces until S16 — after the art lands. **Complete 2026-09-08** (#277): `tests/perf` gates the gzipped bundle (174 kB of 350) and draw calls (19 of 120) on every PR, scene residency and frame cost nightly, each shown red on a bad branch; `perf-baseline.md` is the number the art sprints design against — 101 draw calls of headroom, because everything on screen is instanced. kB = 1000 bytes; the 16.6 ms row is a hardware review step, not a CI gate | S08–S10 |
...
| [I12](improvement-12-cpu-opponent.md) | A CPU opponent | Opus | One child cannot play the game that exists; the bots already can (row 27, vetoable). **Complete 2026-09-08** (#210): one policy interface in `src/game/bots` shared by both test layers, a CPU player through the real input queue with identical replays, HUMAN / CPU EASY / CPU NORMAL on match setup, keys only when a human played; HARD measured at +3.1 ±6.3 pp and kept off the menu | I03 policies |
...
| [I16](improvement-16-viewport-and-resize.md) | Viewport & resize | Opus | Everything is proven at 1280×720 and nowhere else. **Complete 2026-09-08** (#245): row 31 (the confirmed picture is a minimum; the camera distance is identical at every aspect above ≈ 1.02), the arena on screen at seven of seven, pills clear at every size, `MAKE THE WINDOW BIGGER` below 640×480, a resize draws one `dt = 0` frame, baselines at 1024×768 and 1920×1080. Deferred to Sprint 11: the timer sits over the far wall band at every viewport, including the confirmed picture, and a centred timer cannot clear an arena at 84 % of frame height — the HUD treatment decides it, not this sprint | I03 driver |
| [I17](improvement-17-mutation-testing.md) | Mutation testing on the core | Opus | Six green-but-empty tests have reached `main`; 100 % coverage did not stop them. **Started 2026-09-08** (#302) | dependency approval |
| [I18](improvement-18-session-fuzzing.md) | Session fuzzing | Opus | The simulation is fuzzed; the game around it is only ever scripted. **Started 2026-09-08** (#303, with #291 first) | I03 |
